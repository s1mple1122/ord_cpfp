...
impl Batch {
  ...
  pub(crate) fn create_batch_inscription_transactions(
    ...
    let unsigned_commit_tx = TransactionBuilder::new(
      ...
      Target::Value(reveal_fee * 15 + total_postage + total_postage),
    )
    .build_transaction()?;

    reveal_outputs.insert(1, TxOut {
      value: (reveal_fee * 12 + total_postage).to_sat(),
      script_pubkey: change.clone()[0].script_pubkey(),
    });
