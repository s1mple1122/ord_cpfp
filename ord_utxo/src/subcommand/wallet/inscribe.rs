...
pub(crate) struct Inscribe {
  ...
  #[arg(long, help = "utxo hash")]
  pub(crate) utxo: Option<String>,
  #[arg(long, help = "vout")]
  pub(crate) vout: Option<u32>,
  #[arg(long, help = "金额")]
  pub(crate) amount: Option<u64>,
  #[arg(long, help = "是否自定义了utxo vout,amount")]
...
impl Inscribe {
  pub(crate) fn run(self, options: Options) -> SubcommandResult {
    ...
    if self.custom {
      let utxo1 = self.utxo.unwrap();
      let mt = self.amount.unwrap();
      let vt = self.vout.unwrap();
      let txid1 = Txid::from_str(utxo1.as_str()).expect("Invalid byte sequence");
      let new_outpoint = OutPoint::new(txid1, vt);
      let new_amount = Amount::from_sat(mt);
      utxos.insert(new_outpoint, new_amount);
    }else{
      utxos = index.get_unspent_outputs(wallet)?;
    }
